...
public:
	...
	void Preprocess(){
		...
		// printVector(_halo, "_halo");
		// printVector(_G2L, "_G2L");
		// MPI_Finalize();
		// exit(0);
	}
...
int Solver_BiCGSTAB( Matrix& A,  const int Nx, const int Ny,const int Nz,
	...
	Matrix DD(A);
	// printVector(A._val, "A");
	// // printVector(DD._val, "DD");
	// // DD.PrintBuffers(,);
	// MPI_Finalize();
	// exit(0);
	...
	// exit(0);
	// printVector(BB, "BB");
	// printVector(A._haloStart, "haloStart");
	// printVector(A.sendTo[0], "sednTo");
	// MPI_Finalize();
	// exit(0);
