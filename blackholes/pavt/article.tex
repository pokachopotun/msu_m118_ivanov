...
one non-basic node.
First, we need to know what special nodes there are. To find out, we will take every global root of condensated graph and find topoological sort of its closure.
Those vertices, which suffice the rule above, we will mark as "special". 
This step can be precalculated.
Now, we shall calculate how many special nodes for every root there is in the given set. If there is more than one special node for root - the set is to be called non-basis and
omitted. Also we shall check if there are any non-special nodes lower than the special ones, and if those found - omit the node set.
...
TopSortFilter algorithm:
\begin{enumerate}
    \item For every node 
    \item Calculate topological sort order
    \item Calculate closure size for every vertex
    \item For every node
\end{enumerate}
%
