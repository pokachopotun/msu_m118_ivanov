...
we can iterate by $i$ over these nodes in topologically sorted order $topsortOrder(r)[i]$. Also, while traversing this graph, we can calculate size of closure for each node
...
conclude, that every node with index less than $i$ in the topological sort will be reachable from $topsortOrder(r)[i]$. We will call such nodes special.
