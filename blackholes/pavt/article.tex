...
\keywords{Directed networks $\cdot$ Subgraph mining $\cdot$ Blackhole}
\end{abstract}
...
Hong et al \cite{hong2015detecting} consider application for online blackhole mining in the city environment. They introduced an original algorithm an approximate solution of the problem for dynamic graphs.  
...
First, consider a directed graph $G = G(V,E)$, where $V$ is a set of nodes and $E$ is a set if edges of the graph. 
...
\end{theorem}
\begin{proof}
    According to the previous theorem, if $v$ dominates $u$ under root $r$, then $u$ is reachalbe from $v$. Then $u \in Closure(v)$,
...
Third, when we are out of cheap heuristic, we check if any of the candidate nodes are reachable from one another. If so, we omit this set in the high-level TopSort (Algorithm \ref{alg:topsort}).
...
The TopSort algorithm is based on looking over blackhole basises. Thus we have a valid basis of a blackhole.
