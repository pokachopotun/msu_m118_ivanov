...
def PlotGraphFromScale(data, graph, figid):
    ...
    for algo in ["topsort", "chinese"]:
        ...
        for cond in ["cond",]:
            for skipMode in [0, 1]:
                dgc = dga[18][cond][skipMode]
                if algo == "iBlackholeDC":
                    skipMode = "NoSkip"
                if skipMode == 0:
                    skipMode = "SkipPrecise"
                if skipMode == 1:
                    skipMode = "SkipFast"
                scales = dgc.keys()
                values = dgc.values()
                if len(scales) > 0 and len(scales) == len(values):
                    ...
                    top.plot(scales, values, label = "{}.{}".format(algo,  skipMode), linewidth = 3)
                    top.grid(True)
...
def PlotSpeedup(data, graph, figid):
    ...
    scales = values = [x for x in range(1, 141)]
